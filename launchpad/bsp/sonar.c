...
void sonar_init()
{
    ...
    ROM_GPIOIntTypeSet(GPIO_PORTC_BASE, GPIO_PIN_0,GPIO_BOTH_EDGES);
    GPIOIntRegister(GPIO_PORTC_BASE,SonarinputInt);
}
...
void SonarinputInt(){
  ...
  */
      ...
      _pulse = ROM_TimerValueGet(TIMER2_BASE,TIMER_A); //record value
...
void sonar_send_pulse(void)
{
    ...
    ROM_GPIOPinWrite(GPIO_PORTG_BASE, GPIO_PIN_0, GPIO_PIN_0);
    ROM_SysCtlDelay(266);
    ROM_GPIOPinWrite(GPIO_PORTG_BASE, GPIO_PIN_0, ~GPIO_PIN_0);
}
